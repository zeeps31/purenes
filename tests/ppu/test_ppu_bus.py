...
def test_read_from_vram(ppu_bus: ppu.PPUBus):
...
def test_read_from_palette_vram(ppu_bus: ppu.PPUBus):
...
def test_write_to_vram(ppu_bus: ppu.PPUBus):
...
def test_write_to_palette_vram(ppu_bus: ppu.PPUBus):
...
def test_palette_reads_mirror_background(ppu_bus: ppu.PPUBus):
...
def test_palette_writes_mirror_background(ppu_bus: ppu.PPUBus):
...
def test_read_from_an_incorrect_address_is_invalid(ppu_bus: ppu.PPUBus):
    ...
    assert str(exception.value) == INVALID_ADDRESS_EXCEPTION_MESSAGE


def test_write_to_an_incorrect_address_is_invalid(ppu_bus: ppu.PPUBus):
