...
def test_write_to_control_register(ppu: purenes.ppu.PPU, data: int):
...
@pytest.mark.parametrize("data", list(range(0x00, 0xFF)))
def test_write_to_mask_register(ppu: purenes.ppu.PPU, data: int):
...
def test_read_from_status_register_resets_write_latch(ppu: purenes.ppu.PPU):
    """Test PPUSTATUS $2002
...
@pytest.mark.parametrize("data", list(range(0x00, 0xFF)))
def test_write_to_scroll_register(ppu: purenes.ppu.PPU, data: int):
...
@pytest.mark.parametrize("data", list(range(0x00, 0xFF)))
def test_write_to_vram_address_register(ppu: purenes.ppu.PPU, data: int):
