...
class CPU(object):
    ...
    def _execute_comparison_instruction(self, value: int) -> None:
        ...
        self._set_negative_flag(result)
    ...
    def _LSR(self):
        ...
        # The 1st bit of the operation value is preserved in the carry flag.
        self.status.flags.carry = self.operation_value & 0x01

        self.operation_value = (self.operation_value >> 1) & 0x00FF
        self._write_operation_result(self.operation_value)

        # Will always be zero since a zero bit has been shifted into the MSB.
        self._set_negative_flag(self.operation_value)
        self._set_zero_flag(self.operation_value)
    ...
    def _CMP(self):
        ...
        self._execute_comparison_instruction(self.a)

    def _CPX(self):
        # Compare Memory and Index X
        self._execute_comparison_instruction(self.x)

    def _CPY(self):
        # Compare Memory and Index Y
        self._execute_comparison_instruction(self.y)
    ...
    def _RTS(self):
        ...
        lo: int = self._pull_from_stack()
        hi: int = self._pull_from_stack()
    ...
    def _map_operations(self) -> None:
        ...
        self._operations = {
            ...
            0x46: (op._zpg, op._LSR, 5), 0x48: (op._imp, op._PHA, 3),
            0x49: (op._imm, op._EOR, 2), 0x4A: (op._acc, op._LSR, 2),
            0x4D: (op._abs, op._EOR, 4), 0x4E: (op._abs, op._LSR, 6),
            0x50: (op._rel, op._BVC, 2), 0x51: (op._izy, op._EOR, 5),
            0x55: (op._zpx, op._EOR, 4), 0x56: (op._zpx, op._LSR, 6),
            0x58: (op._imp, op._CLI, 2), 0x59: (op._aby, op._EOR, 4),
            0x5D: (op._abx, op._EOR, 4), 0x5E: (op._abx, op._LSR, 7),
            0x60: (op._imp, op._RTS, 6), 0x61: (op._izx, op._ADC, 6),
            0x65: (op._zpg, op._ADC, 3), 0x66: (op._zpg, op._ROR, 5),
            0x68: (op._imp, op._PLA, 4), 0x69: (op._imm, op._ADC, 2),
            0x6A: (op._acc, op._ROR, 2), 0x6C: (op._ind, op._JMP, 5),
            0x6D: (op._abs, op._ADC, 4), 0x6E: (op._abs, op._ROR, 6),
            0x70: (op._rel, op._BVS, 2), 0x71: (op._izy, op._ADC, 5),
            0x75: (op._zpx, op._ADC, 4), 0x76: (op._zpx, op._ROR, 6),
            0x78: (op._imp, op._SEI, 2), 0x79: (op._aby, op._ADC, 4),
            0x7D: (op._abx, op._ADC, 4), 0x7E: (op._abx, op._ROR, 7),
            ...
            0xBE: (op._aby, op._LDX, 4), 0xC0: (op._imm, op._CPY, 2),
            0xC1: (op._izx, op._CMP, 6), 0xC4: (op._zpg, op._CPY, 3),
            0xC5: (op._zpg, op._CMP, 3), 0xC6: (op._zpg, op._DEC, 5),
            0xC8: (op._imp, op._INY, 2), 0xC9: (op._imm, op._CMP, 2),
            0xCA: (op._imp, op._DEX, 2), 0xCC: (op._abs, op._CPY, 4),
            0xCD: (op._abs, op._CMP, 4), 0xCE: (op._abs, op._DEC, 6),
            0xD0: (op._rel, op._BNE, 2), 0xD1: (op._izy, op._CMP, 5),
            0xD5: (op._zpx, op._CMP, 4), 0xD6: (op._zpx, op._DEC, 6),
            0xD8: (op._imp, op._CLD, 2), 0xD9: (op._aby, op._CMP, 4),
            0xDD: (op._abx, op._CMP, 4), 0xDE: (op._abx, op._DEC, 7),
            0xE0: (op._imm, op._CPX, 2), 0xE1: (op._izx, op._SBC, 6),
            0xE4: (op._zpg, op._CPX, 3), 0xE5: (op._zpg, op._SBC, 3),
            0xE6: (op._zpg, op._INC, 5), 0xE8: (op._imp, op._INX, 2),
            0xE9: (op._imm, op._SBC, 2), 0xEC: (op._abs, op._CPX, 4),
